...
#define CONFIG_SPIFFS_BASE_ADDR 0x2c0004
#define CONFIG_SPIFFS_SIZE 0x13fff0
...
static int IRAM_ATTR vfs_spiffs_open(const char *path, int flags, int mode) {
    ...
    int res = flist_add(&files, file, &fd);
    if (res) {
    	free(file);
    	errno = res;
    	return -1;
...
static ssize_t IRAM_ATTR vfs_spiffs_write(int fd, const void *data, size_t size) {
    ...
    res = flist_get(&files, fd, (void **) &file);
    if (res) {
		errno = EBADF;
		return -1;
    }
